...
# beta1 <- runif(n.p)-0.5
# beta2 <- runif(n.p)-0.5
Y1 <- (X[, 15] < -0.5 | X[, 20] < -0.5) + 0.5*X[, 2] + 0.5*X[, 4] - X[, 6] + X[, 8]*X[, 10]
Y2 <- 1.5*(X[, 15] >= -0.5 & X[, 20] >= -0.5) + 0.5*X[, 2] + 0.5*X[, 4] - X[, 6] + X[, 8]*X[, 10]
...
dnn.mod <- dnnet(split.dat$train, validate = split.dat$valid,
                 ...
                 l1.reg = 10**-4, l2.reg = 0, n.batch = 10, n.epoch = 500,
...
for(i in 1:n.ensemble) {
  ...
  dnn.mod <- dnnet(split.dat$train, validate = split.dat$valid,
                   ...
                   early.stop = TRUE, early.stop.det = 100, plot = TRUE, accel = "rcpp",
                   learning.rate.adaptive = "adagrad")
  pred.table[, i] <- predict(dnn.mod, split.dat$train@x)
...
plot(dat@y ~ y.pred)
abline(0, 1)
plot((Y1[1:n.sample] + Y2[1:n.sample])/2 ~ y.pred)
abline(0, 1)
...
dnn.mod <- dnnet(split.dat0$train, validate = split.dat0$valid,
                 norm.x = FALSE, norm.y = FALSE,
                 activate = "elu", n.hidden = c(10, 10, 10), learning.rate = 0.01,
